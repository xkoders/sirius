...
<template>
  <div class="relative">
        ...
        <thead class="bg-slate-200/70">
          <tr>
            <th v-if="tableResources.select" class=" text-sm text-gray-900 align-middle">
              <Checkbox :checked="checkboxType === 'all'" :indeterminate="checkboxType === 'intermediate'"
    ...
    <Transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
      ...
      <div v-if="tableResources.selectedItems?.length > 0"
        class="absolute top-full translate-y-1 left-1/2 -translate-x-1/2 z-50 bg-white p-4 shadow-lg rounded-md border flex gap-4 h-fit">
        <div v-if="bulkActions" class="flex">
          <button v-for="(action, index) in bulkActions" :key="index" @click="action.onAction"
        ...
        <Popover v-if="promotedBulkActions" class="relative h-9">
          ...
          <div
            class="absolute z-10 bg-white shadow bottom-11 w-40 rounded-md flex flex-col p-3 right-0 text-gray-600 text-[15px]">
            <button v-for="(action, index) in promotedBulkActions" :key="index" @click="action.onAction"
