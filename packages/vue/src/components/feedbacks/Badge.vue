  ...
  >
    <div v-if="!!type || !!icon" class="-ml-1 flex">
      <svg v-if="type" viewBox="0 0 200 200" class="w-2.5" fill="none">
        <circle v-if="type === 'incomplete'" cx="100" cy="100" r="89.5" stroke="black" stroke-width="21" />
        <path
          v-else-if="type === 'halfComplete'"
          fill-rule="evenodd"
          clip-rule="evenodd"
          ...
          fill="currentColor"
        />
        <circle v-else-if="type === 'complete'" cx="100" cy="100" r="100" fill="currentColor" />
        <circle
      ...
      <div v-if="icon">
        <component :is="icon" class="w-4 h-4 fill-current" />
      </div>
    </div>
    <slot />
  </div>
